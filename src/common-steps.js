...
function steps({Before, After, Given, When, Then}) {
  ...
  Then('I should see the page {string}', pageTitle => {
    ui.waitReady()
    cy.get('div.pageTitle').contains(pageTitle).should('be.visible')
  })


  Then('I should see data in the status bar', () => {
    ui.waitReady()
    const rowCountTitle = 'Filtered rows and total number of rows in data table.'
    cy.get(`div[title="${rowCountTitle}"]`).as('filterStatus').then(e => {
      const content = e.text()
      if (content === '0 of 0 rows') {
        throw new Error('The data is not loaded in the analysis.')
      }
    })
  })
